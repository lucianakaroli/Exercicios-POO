...
export class Turma{
    ...
    aprovados() {
        ...
        const nomeAprovados = pipe(filterAprovados, map(getNome));
        return nomeAprovados(this.#alunos);
    }


    reprovados() {
        return filter(a => !a.aprovado(), this.#alunos);
    }


    resultadoFinal(){
        return map(a => ({nome: a.nome, media: a.media(), aprovado: a.aprovado()}), this.#alunos);
    }
